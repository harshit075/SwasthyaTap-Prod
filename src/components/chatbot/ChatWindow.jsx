...
import SuggestedQuestions, { SUGGESTIONS } from './SuggestedQuestions';
...
export default function ChatWindow({
  ...
  // Compute remaining suggestions
  const userMsgTexts = messages
    .filter(m => m.role === 'user')
    .map(m => m.content.toLowerCase().trim());

  const remainingSuggestions = SUGGESTIONS.filter(q => 
    !userMsgTexts.includes(q.toLowerCase().trim())
  );
      ...
      >
        {messages.length === 0 ? (
          <div className="flex-1 flex flex-col justify-between py-6">
            ...
            <SuggestedQuestions onSelectQuestion={(q) => onSendMessage(q)} suggestions={remainingSuggestions} />
          </div>
        ) : (
          <div className="flex flex-col py-2">
            ...
            {!isLoading && remainingSuggestions.length > 0 && (
              <SuggestedQuestions 
